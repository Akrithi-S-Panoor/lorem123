...
//    public User getPass()
//    {
//        return jdbcTemplate.queryForObject("select password from user where userId=21",new BeanPropertyRowMapper<>(User.class));
//    }
    ...
    public Map<Integer,List<offerSelectiveFields>> viewBrandOffers(int brandId, int page)
    {
        Map map = new HashMap<Integer,List>();
        offset=limit*(page-1);
...
//            userId have to take when user logged in
    ...
    public Map<Integer,List<offerSelectiveFields>> viewBestOfferOfRestaurant(int page,int restaurantId)
    {
        //offset = limit*(pg-1) --> 3*(1-1)
        offset=limit*(page-1);
        Map map = new HashMap<Integer,List>();
        ...
        try
        {
            ...
            if(brandId!=0)
            {
                List <offerSelectiveFields> users= jdbcTemplate.query("select offerId,discount,description,photo from offer where brandId=? order by discount desc limit ?,?", new BeanPropertyRowMapper<>(offerSelectiveFields.class),brandId,offset,limit);
        ...
        {
            List <offerSelectiveFields> users= jdbcTemplate.query(GET_OFFERS_OF_RESTAURANT, new BeanPropertyRowMapper<>(offerSelectiveFields.class),offset,limit);
    ...
    public Map<Integer,List<offerSelectiveFields>> viewAllOffersOfRestaurant(int page,int restaurantId)
    {
        //offset = limit*(pg-1) --> 3*(1-1)
        offset=limit*(page-1);
        Map map = new HashMap<Integer,List>();
        ...
        try
        {
            ...
            if(brandId!=0)
            {
                List <offerSelectiveFields> users= jdbcTemplate.query("select offerId,discount,description,photo from offer where brandId=?  limit ?,?", new BeanPropertyRowMapper<>(offerSelectiveFields.class),brandId,offset,limit);
        ...
        {
            List <offerSelectiveFields> users= jdbcTemplate.query("select offerId,discount,description,photo from offer where brandId is null limit ?,?", new BeanPropertyRowMapper<>(offerSelectiveFields.class),offset,limit);
